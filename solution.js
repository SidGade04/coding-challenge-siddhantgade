...
const total = eventDetails
  .filter(e => idsA.includes(e.id_a) || idsB.includes(e.id_b))
  .reduce((sum, e) => sum + e.value, 0);

// ---------------------------- Task2b: Earliest date & min value → names ----------------------------
const earliestDateName = eventDetails
    .filter(e => idsA.includes(e.id_a) || idsB.includes(e.id_b))
    .map(e => ({date: e.date, name: references.find(r => r.id_a === e.id_a || r.id_b === e.id_b)?.name}))
    .sort((a, b) => new Date(a.date) - new Date(b.date))[0].name;

const minValueName = eventDetails
  .filter(e => idsA.includes(e.id_a) || idsB.includes(e.id_b))
  .map(e => ({value: e.value, name: references.find(r => r.id_a === e.id_a || r.id_b === e.id_b)?.name}))
  .sort((a, b) => a.value - b.value)[0].name;

// ---------------------------- Task2c: High value threshold filter ----------------------------
const valueGreaterThan25Names = eventDetails
    .filter(e => idsA.includes(e.id_a) || idsB.includes(e.id_b))
    .filter(e => e.value > 25)
    .map(e => ({value: e.value, name: references.find(r => r.id_a === e.id_a || r.id_b === e.id_b)?.name}))
